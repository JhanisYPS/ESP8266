...
<head>
    ...
    <title>Detecção de Arritmia</title>
...
<body>
    ...
    <script>
        ...
        function getHeartRateVal() {
            // Simule a geração de dados do BPM (substitua isso pela sua lógica real)
            const bpm = Math.floor(Math.random() * (200 - 60) + 60); // Exemplo: BPM aleatório entre 60 e 200
            ...
            // Envie os dados para o servidor Python (substitua esta parte pela sua lógica real)
            enviarDadosParaPython(bpm);
        }
        ...
        // Função para enviar dados para o servidor Python (substitua esta parte pela sua lógica real)
        function enviarDadosParaPython(bpm) {
            fetch('/analyze', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                // Processar a resposta do servidor (por exemplo, exibir o resultado na página)
